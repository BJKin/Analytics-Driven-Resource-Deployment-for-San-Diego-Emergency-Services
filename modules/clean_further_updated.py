...
def load_and_clean_raw(
    ...
    drop_intersecting_cols: bool = True,
) -> pd.DataFrame:
    """
    Load the raw Police Calls-for-Service CSV and perform initial cleaning:
      1. Parse DATE_TIME as datetime.
      2. Drop intersecting-street columns (ADDRESS_DIR_INTERSECTING,
         ADDRESS_ROAD_INTERSECTING, ADDRESS_SFX_INTERSECTING).
      3. Drop rows where ADDRESS_ROAD_PRIMARY, CALL_TYPE, or DISPOSITION is null.
    Returns a cleaned DataFrame (equivalent to the notebook's df_cleaned).
    """
    raw_path = Path(raw_csv_path)
    assert raw_path.exists(), f"Raw CSV not found: {raw_path}"
    ...
    # Drop intersecting-street columns if they exist
    if drop_intersecting_cols:
        intersecting_cols = [
            "ADDRESS_DIR_INTERSECTING",
            "ADDRESS_ROAD_INTERSECTING",
            "ADDRESS_SFX_INTERSECTING",
        ]
        cols_to_drop = [c for c in intersecting_cols if c in df.columns]
        if cols_to_drop:
            df = df.drop(columns=cols_to_drop)
            print(f"Dropped columns: {cols_to_drop}")

    # Drop rows with missing critical fields
    required_non_null = ["ADDRESS_ROAD_PRIMARY", "CALL_TYPE", "DISPOSITION"]
    present = [c for c in required_non_null if c in df.columns]
    before = len(df)
    df = df.dropna(subset=present).reset_index(drop=True)
    print(f"Dropped {before - len(df)} rows with missing {present}")
...
def main() -> None:
    ...
    df_cleaned = load_and_clean_raw(raw_csv_path)
