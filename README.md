...
- •	Navigate to the root directory and the `step3_forecasting.py` script:
    ```bash
    ...
    - Core Pipeline Functions (Callled by step3_forecasting.py):
...
- Navigate to the root directory and run the `step4_scenario_analysis.py` script
    ```bash
    python modules/step4_scenario_analysis.py
    ```
    - Generates multiple deployment policies (scenarios), evaluates them with **proxy operational metrics** (coverage/shortfall under a simplified capacity assumption), and outputs **CSV summaries + plots** for reporting.
        - Input
            - `data/01-processed/step4_resource_deployment.csv` 
                - Determines the baseline demand distribution; all scenarios allocate units based on this table
            - `data/01-processed/step3_hotspots_beats.csv` 
                - Changes which beats receive minimum guaranteed units under hotspot-protect
        - Output
            - Outputs written to `data/01-processed/step4_scenarios/`
                - `step4_scenario_allocations.csv` 
                    - Beat × shift × scenario **allocation detail table**. Each row is one `(BEAT, SHIFT, SCENARIO)` cell with allocated `UNITS` and proxy KPIs such as `CAPACITY`, `COVERAGE`, and `SHORTFALL`.
